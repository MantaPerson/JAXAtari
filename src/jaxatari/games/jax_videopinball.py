...
def ball_step(
    ...
    ball_vel_x = jnp.where(jnp.logical_and(any_collision, small_vel), jnp.clip(ball_vel_x * 3 + 5e-1, 0, BALL_MAX_SPEED), ball_vel_x)
    ball_vel_y = jnp.where(jnp.logical_and(any_collision, small_vel), jnp.clip(ball_vel_y * 3 + 5e-1, 0, BALL_MAX_SPEED), ball_vel_y)
